...
def create_appointment(client, patient_id: str, headers: dict[str, str] | None = None) -> dict:
    response = client.post(
        ...
        headers=headers or DOCTOR_HEADERS,
    )
    assert response.status_code == 201
    return response.json()
...
def test_timeline_aggregates_mixed_events_and_orders_descending(client, create_user) -> None:
    ...
    timeline_response = client.get(f"/api/v1/patients/{patient['id']}/timeline", headers=DOCTOR_HEADERS)
...
def test_timeline_authorization_behavior(client, doctor_headers, unsupported_role_headers) -> None:
    ...
    doctor_allowed = client.get(f"/api/v1/patients/{patient_id}/timeline", headers=doctor_headers)
    assert doctor_allowed.status_code == 200


def test_timeline_returns_404_for_unknown_patient(client) -> None:
    response = client.get("/api/v1/patients/00000000-0000-0000-0000-000000000000/timeline", headers=DOCTOR_HEADERS)
...
def test_timeline_can_be_empty_when_no_events_exist(client, db_session) -> None:
    ...
    response = client.get(f"/api/v1/patients/{patient.id}/timeline", headers=DOCTOR_HEADERS)
